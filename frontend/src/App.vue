...
if (isAuthed.value && !auth.me) auth.fetchMe().catch(() => { /* 401 由拦截器处理 */ })
watch(isAuthed, (v) => { if (v && !auth.me) auth.fetchMe().catch(() => {}) })
</script>

<template>
  <div class="stage">
    <div class="canvas" :class="{ zoom: canvasZoom }">
      <div id="sv-layer" />
      <SvCrisisBanner v-if="isAuthed && route.path !== '/crisis'" />
...
.canvas {
  ...
  background: var(--sv-bg); color: var(--sv-label);
  display: flex; flex-direction: column; overflow: hidden;
}
.canvas.zoom .screen { transform: scale(0.96); }
@media (min-width: 1024px) {
  body { overflow: hidden; }
  .canvas {
    width: 430px; height: min(908px, 100dvh); min-height: 0;
    border-radius: 44px;
    box-shadow: 0 40px 120px rgba(0, 0, 0, 0.6), 0 0 0 10px #16181f;
  }
}
.screen { flex: 1; min-height: 0; padding-bottom: 20px; transition: transform 0.35s var(--sv-ease); }
#sv-layer { position: absolute; inset: 0; z-index: 100; pointer-events: none; }
#sv-layer > * { pointer-events: auto; }

/* 路由转场 */
.fade-enter-active, .push-enter-active, .pop-enter-active { transition: opacity 0.24s ease, transform 0.32s var(--sv-ease); }
.fade-leave-active, .push-leave-active, .pop-leave-active { transition: opacity 0.18s ease, transform 0.32s var(--sv-ease); }
.fade-enter-from, .fade-leave-to { opacity: 0; }
.push-enter-from { opacity: 0; transform: translateX(24%); }
.push-leave-to { opacity: 0; transform: translateX(-18%); }
.pop-enter-from { opacity: 0; transform: translateX(-18%); }
.pop-leave-to { opacity: 0; transform: translateX(24%); }
</style>
