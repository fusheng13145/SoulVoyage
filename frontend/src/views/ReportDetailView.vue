...
interface ReportDetail {
  id: number; type: string; title: string; riskLevel: string; createdAt: string
  content: any
}
...
const DIM_LABELS: Record<string, string> = { LISTEN: '倾听', BOUNDARY: '边界表达', EMPATHY: '共情', CONCESSION: '让步策略' }
const gradeCls = (g: string) => ({ A: 'g-a', B: 'g-b', C: 'g-c', D: 'g-d' }[g] || 'g-c')

const isTrace = computed(() => report.value && (report.value.type === 'TRACE' || report.value.type === 'DIARY_TRACE'))
const isDiaryReport = computed(() => report.value?.type === 'DIARY_REPORT')
const isSupport = computed(() => report.value?.type === 'SUPPORT')
const isSimulate = computed(() => report.value && (report.value.type === 'SIMULATE' || report.value.type === 'SIMULATE_REVIEW'))
const riskChip = computed(() => ({
  HIGH: { label: '需要关照', tone: 'color-mix(in srgb, var(--sv-red) 16%, transparent)', color: 'var(--sv-red)' },
  MEDIUM: { label: '情绪起伏', tone: 'color-mix(in srgb, var(--sv-amber) 18%, transparent)', color: 'var(--sv-amber)' },
} as Record<string, { label: string; tone: string; color: string }>)[report.value?.riskLevel || '']
  || { label: '平稳', tone: 'color-mix(in srgb, var(--sv-mint) 14%, transparent)', color: 'var(--sv-mint)' })

const exerciseNames = computed(() =>
  Object.fromEntries((content.exercises ?? []).map(x => [x.id, x])))

/** 溯源报告正文可能挂在 content 顶层或 content.report */
const traceBody = computed(() => report.value?.content?.report ?? report.value?.content ?? {})

onMounted(async () => {
  content.ensureExercises().catch(() => { /* 名称降级为 id */ })
  try {
    const { data } = await http.get<ApiResp<ReportDetail>>(`/reports/${route.params.id}`)
    report.value = data.data
  } catch (e: any) {
    toast(e.message || '报告不存在或已删除')
  } finally {
    loading.value = false
  }
})
...
<template>
  <div class="report">
    ...
    <div class="body" v-if="report">
      <SvCard>
        <div class="head">
          <h2>{{ report.title }}</h2>
          <SvChip :model-value="true" :tone="riskChip.tone" :color="riskChip.color" class="ro">{{ riskChip.label }}</SvChip>
        </div>
        <p class="sv-cap">{{ report.createdAt ? report.createdAt.slice(0, 16).replace('T', ' ') : '' }} · {{ TYPE_LABELS[report.type] || report.type }}</p>
      </SvCard>
          ...
          <dl class="report-dl">
            <template v-for="(v, k) in (traceBody.report || traceBody)" :key="k">
              <template v-if="REPORT_TITLES[k as string] && typeof v === 'string'">
                <dt>{{ REPORT_TITLES[k as string] }}</dt><dd>{{ v }}</dd>
              </template>
            </template>
          </dl>
        </SvCard>
        <SvCard v-if="traceBody.stressors?.length">
          ...
          <ul class="plain">
            <li v-for="s in traceBody.stressors" :key="s.source">
              <b>{{ s.source }}</b><em class="conf">{{ Math.round((s.confidence ?? 0) * 100) }}%</em>
      ...
      <SvCard v-if="isSupport">
        ...
        <div v-for="m in report.content.matchedExercises" :key="m.exerciseId" class="plan-item">
          ...
          <small v-if="exerciseNames[m.exerciseId]" class="sv-muted">
            {{ exerciseNames[m.exerciseId].steps.map((s: any) => s.step).join(' → ') }} ·
            约 {{ exerciseNames[m.exerciseId].durationMin }} 分钟</small>
        </div>
      ...
      <SvDisclaimer text="报告基于你自己的记录由 AI 生成，是自助参考，不构成任何诊断"
...
.head h2 { font-size: var(--sv-fs-title3); margin: 0; }
.ro { cursor: default; height: auto; flex: none; }
.t { font-size: var(--sv-fs-callout); margin-bottom: var(--sv-s2); }
.report-dl { font-size: var(--sv-fs-subhead); }
.report-dl dt { float: left; clear: left; width: 4.5em; color: var(--sv-label2); }
.report-dl dd { margin: 0 0 8px 5em; line-height: 1.7; }
.plain li { padding: 8px 0; border-bottom: 1px dashed var(--sv-sep); font-size: var(--sv-fs-subhead); }
.plain li:last-child { border-bottom: none; }
.conf { font-style: normal; margin-left: 8px; font-size: var(--sv-fs-caption1); color: var(--sv-indigo); background: var(--sv-indigo-soft); border-radius: var(--sv-r-pill); padding: 1px 8px; }
.ev { display: block; font-size: var(--sv-fs-caption1); }
.dist { border: 1px solid var(--sv-sep); border-radius: var(--sv-r-ctl); padding: 10px 12px; margin-bottom: var(--sv-s2); font-size: var(--sv-fs-subhead); }
.dist .q { margin-top: 6px; }
.socratic { padding-left: 22px; list-style: decimal; font-size: var(--sv-fs-subhead); line-height: 1.8; }
.plan-item { border: 1px solid var(--sv-sep); border-radius: var(--sv-r-ctl); padding: 10px 12px; margin-bottom: var(--sv-s2); font-size: var(--sv-fs-subhead); }
.pi-head { display: flex; justify-content: space-between; align-items: baseline; }
.plan-item b { color: var(--sv-mint); }
.plan-item p { margin: 4px 0; }
.when { font-style: normal; font-size: var(--sv-fs-caption1); background: var(--sv-indigo-soft); color: var(--sv-indigo); border-radius: var(--sv-r-pill); padding: 1px 8px; }
...
.avg { font-size: 40px; font-weight: 700; color: var(--sv-indigo); font-variant-numeric: tabular-nums; }
.advice { margin: var(--sv-s2) 0; font-size: var(--sv-fs-subhead); line-height: 1.7; }
.dim-scores { display: flex; gap: 6px; margin-bottom: var(--sv-s3); flex-wrap: wrap; }
.dim-chip { font-size: var(--sv-fs-caption1); background: var(--sv-fill2); color: var(--sv-label2); border-radius: var(--sv-r-pill); padding: 2px 10px; }
h4 { margin: var(--sv-s4) 0 var(--sv-s2); font-size: var(--sv-fs-footnote); color: var(--sv-label2); }
...
.toggle { border: none; background: var(--sv-fill2); color: var(--sv-indigo); border-radius: var(--sv-r-pill); padding: 8px 16px; cursor: pointer; font-family: inherit; font-size: var(--sv-fs-footnote); }
.raw { white-space: pre-wrap; word-break: break-all; margin-top: var(--sv-s3); font-size: var(--sv-fs-caption1); }
