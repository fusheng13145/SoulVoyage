...
async function patch(body: Partial<Prefs>) {
  ...
  try {
    ...
    toast((e as Error).message || '设置没保存上，再试一次')
  }
}
    ...
    <div class="body">
      ...
      <SvList title="屿上的痕迹">
        <SvCell
          ...
          hint="每周一封 · 只引用你说过的话"
          icon="i-mail"
          tone="color-mix(in srgb, var(--sv-amber) 14%, transparent)"
          to="/letters"
        />
        <SvCell
          label="成就墙"
      ...
      <SvList title="外观与体感">
        ...
        >
          <template #extra
            ><SvSwitch
              :model-value="hapticOn"
              label="触感反馈"
              @update:model-value="toggle('hapticOn', $event)"
          /></template>
        </SvCell>
      </SvList>
      ...
      <SvList title="它如何提醒你">
        ...
        >
          <template #extra
            ><SvSwitch
              :model-value="prefs?.checkinReminderOn ?? false"
              label="打卡提醒"
              :disabled="!prefs"
              @update:model-value="toggle('checkinReminderOn', $event)"
          /></template>
        </SvCell>
        <SvCell v-if="prefs" label="提醒时间" icon="i-bell" tone="var(--sv-indigo-soft)" :chevron="false">
          <template #extra>
            <input
              ...
              aria-label="打卡提醒时间"
              :disabled="!prefs.checkinReminderOn"
              @change="pickTime"
            />
          </template>
        </SvCell>
        ...
        >
          <template #extra
            ><SvSwitch
              :model-value="prefs?.planReminderOn ?? false"
              label="计划提醒"
              :disabled="!prefs"
              @update:model-value="toggle('planReminderOn', $event)"
          /></template>
        </SvCell>
        <SvCell
          label="成长来信"
        ...
        >
          <template #extra
            ><SvSwitch
              :model-value="prefs?.letterOn ?? false"
              label="成长来信"
              :disabled="!prefs"
              @update:model-value="toggle('letterOn', $event)"
          /></template>
        </SvCell>
        <SvCell
          label="漫聊情绪消化"
          hint="关掉后收段只做归档，不做情绪分析；单句仍可用「这句别分析」"
          icon="i-chat"
          tone="color-mix(in srgb, var(--sv-purple, var(--sv-indigo)) 14%, transparent)"
          :chevron="false"
        >
          <template #extra
            ><SvSwitch
              :model-value="prefs?.companionAnalysisOn ?? false"
              label="漫聊分析"
              :disabled="!prefs"
      ...
      <SvList v-if="me?.role === 'ADMIN'" title="运营">
        <SvCell
          label="管理端工作台"
          hint="任务 · 风险 · 内容 · 审计"
          icon="i-grid"
      ...
      <SvList title="支持">
        <SvCell
          ...
          hint="随时可连的热线与渠道"
          icon="i-phone"
          tone="color-mix(in srgb, var(--sv-red) 14%, transparent)"
          to="/crisis"
        />
      </SvList>
