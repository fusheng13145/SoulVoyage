...
function mark(agent: string, state: FlowStep['state'], sub?: string) {
  ...
  if (s) { s.state = state; if (sub) s.sub = sub }
}
...
onMounted(async () => {
  content.ensureExercises().catch(() => { /* 目录失败不挡流程 */ })
    ...
    await streamTask(taskNo, (e) => {
      switch (e.event) {
        ...
        case 'done':
          ...
          break
        case 'error':
          error.value = e.data.message || '服务异常'; running.value = false
      }
    }, abort.signal)
  } catch (e: any) {
    error.value = e.message || '进度连接中断'
  } finally {
    running.value = false
  }
})
...
<template>
  <div class="analysis">
    <SvNavBar title="心屿梳理" back="日记" backTo="/diaries" :large="false">
      <template #actions>
        <router-link to="/diaries" class="ic" aria-label="回日记本"><SvIcon name="i-diary" :size="20" /></router-link>
      </template>
    </SvNavBar>
    ...
    <div class="body">
      ...
      <SvCard v-if="emotion" id="block-EMOTION">
        ...
        <div class="bars">
          <div class="bar"><span>强度</span>
            <div class="track"><div class="fill" :style="{ width: emotion.intensity * 100 + '%' }" /></div>
            <b>{{ Math.round(emotion.intensity * 100) }}%</b></div>
          <div class="bar"><span>效价</span>
            <div class="track"><div class="fill v" :class="emotion.valence < 0 ? 'neg' : 'pos'"
              :style="{ width: Math.abs(emotion.valence) * 100 + '%' }" /></div>
            <b>{{ emotion.valence }}</b></div>
        </div>
        <div class="tags" v-if="emotion.eventTags?.length">
      ...
      <SvCard v-if="trace" id="block-TRACE">
        <h3 class="t">溯源复盘</h3>
        <p v-if="trace.insufficientEvidence" class="sv-muted">这次记录有点少，先不下结论，想到什么再补一句。</p>

        <template v-if="trace.stressors?.length">
          <h4>可能的压力来源</h4>
          <ul class="stressors">
            <li v-for="s in trace.stressors" :key="s.source">
              <b>{{ s.source }}</b><em class="conf">{{ Math.round(s.confidence * 100) }}%</em>
        ...
        <SvDisclaimer class="disc" text="以上为自助梳理参考，不构成任何诊断"
      ...
      <!-- ④ 风险归档回执（仅需要关照时展示） -->
      <CrisisReferral v-if="showCrisis" :level="receipt.riskLevel === 'HIGH' ? 'HIGH' : 'MEDIUM'"
...
.ic { display: grid; place-items: center; width: 44px; height: 44px; color: var(--sv-indigo); }
.err { color: var(--sv-red); font-size: var(--sv-fs-footnote); }
.t { font-size: var(--sv-fs-title3); margin-bottom: var(--sv-s3); display: flex; align-items: baseline; gap: 8px; }
.emo { font-weight: 700; }
.bars { display: flex; flex-direction: column; gap: var(--sv-s2); margin-bottom: var(--sv-s3); }
.bar { display: flex; align-items: center; gap: 10px; font-size: var(--sv-fs-footnote); color: var(--sv-label2); }
.bar span { width: 2.5em; }
.bar b { font-variant-numeric: tabular-nums; color: var(--sv-label); }
.track { flex: 1; height: 8px; background: var(--sv-fill2); border-radius: 4px; overflow: hidden; }
.fill { height: 100%; border-radius: 4px; background: var(--sv-amber); }
.fill.pos { background: var(--sv-mint); } .fill.neg { background: var(--sv-amber); }
.tags { display: flex; flex-wrap: wrap; gap: 8px; }
.tag { font-size: var(--sv-fs-caption1); background: var(--sv-indigo-soft); color: var(--sv-indigo);
  border-radius: var(--sv-r-pill); padding: 4px 10px; }
.tag em { font-style: normal; opacity: 0.75; margin-left: 4px; }
h4 { margin: var(--sv-s4) 0 var(--sv-s2); font-size: var(--sv-fs-footnote); color: var(--sv-label2); font-weight: 600; }
.stressors li { padding: 8px 0; border-bottom: 1px dashed var(--sv-sep); font-size: var(--sv-fs-subhead); }
.stressors li:last-child { border-bottom: none; }
...
.ev { display: block; color: var(--sv-label2); font-size: var(--sv-fs-caption1); margin-top: 2px; }
.dist { border: 1px solid var(--sv-sep); border-radius: var(--sv-r-ctl); padding: 10px 12px; margin-bottom: var(--sv-s2); }
.dist .q { margin-top: 6px; font-size: var(--sv-fs-subhead); }
.socratic { padding-left: 22px; list-style: decimal; color: var(--sv-label); font-size: var(--sv-fs-subhead); line-height: 1.8; }
.report { font-size: var(--sv-fs-subhead); }
.report dt { float: left; clear: left; width: 4.5em; color: var(--sv-label2); }
.report dd { margin: 0 0 8px 5em; }
.disc { margin-top: var(--sv-s4); }
...
.plan-item p { margin: 4px 0; color: var(--sv-label); }
.plan-item small { color: var(--sv-label2); font-size: var(--sv-fs-caption1); }
...
.psy h4 { margin-bottom: 4px; }
.psy p { line-height: 1.7; }
