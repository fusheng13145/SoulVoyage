...
const planItemSeq =
  route.query.planItemSeq || route.query.seq ? Number(route.query.planItemSeq ?? route.query.seq) : undefined
...
async function report(star?: number, feedback?: string) {
  ...
  try {
    ...
    toast((e as Error).message || '打卡没写上，再试一次')
    return false
  } finally {
    saving.value = false
  }
...
function startBreath() {
  ...
  breathTimer = setInterval(() => {
    ...
    if (phaseIdx.value < 2) {
      phaseIdx.value += 1
      phaseLeft.value = PHASES[phaseIdx.value].secs
      return
    }
    cycles.value += 1
    if (cycles.value >= CYCLE_GOAL) {
      stopBreath()
      return
    }
    phaseIdx.value = 0
...
function startGround() {
  ...
  groundTimer = setInterval(() => {
    if (gOn.value && gLeft.value > 0) gLeft.value -= 1
  }, 1000)
}
function stopGround() {
...
const cbtError = ref('')
const cbtAbort: AbortController | undefined = new AbortController()
...
async function submitCbt() {
  ...
  try {
    ...
    cbtError.value = msg || '提交失败，内容还在，再试一次'
    cbtPhase.value = 'form'
  }
}
...
async function finishAction() {
  if (!picked.value.length) {
    toast('先选 1-2 件 5 分钟内能完成的小事')
    return
  }
  if (!stars.value) {
    toast('做完后给此刻心情打个分吧（1-5 星）')
    return
  }
...
<template>
  <div class="room">
    <SvNavBar
      :title="ex?.name || '沉浸跟练'"
      :subtitle="loadError ? '' : `已进行 ${fmtClock} · 约 ${ex?.durationMin || '—'} 分钟`"
      back="练习"
      back-to="/practice"
      :large="false"
    />
    ...
    <div class="body">
      ...
      <template v-else-if="mode === 'breath'">
        <div class="stage">
          ...
          <p class="sv-cap cyc">
            {{
              breathOn
                ? `第 ${Math.min(cycles + 1, CYCLE_GOAL)} / ${CYCLE_GOAL} 轮 · ${phase.name} ${Math.max(0, phaseLeft)} 秒`
                : '跟随光圈呼吸，不用憋得太勉强'
            }}
          </p>
          <div class="acts">
        ...
        <div class="stage">
          <template v-if="!groundDone">
            ...
            <div v-else class="grounding" aria-live="polite">
              ...
              <p class="sv-cap gtime">
                {{ gLeft > 0 ? `慢慢来，还有 ${gLeft} 秒` : '可以继续，也可以走下一步' }}
              </p>
            </div>
            <div class="acts">
              <button v-if="!gOn" class="sv-btn" @click="startGround">开始着陆</button>
              <button v-else class="sv-btn" @click="groundNext">
                {{ gIdx < 4 ? `下一步 · 听 ${GROUND[gIdx + 1].n} 种` : '我做到了，完成' }}
              </button>
          ...
          <template v-else>
            ...
            <div class="acts">
              <button v-if="!finished" class="sv-btn" @click="report()">打卡这一分钟</button>
            </div>
          </template>
        </div>
      </template>
      ...
      <template v-else-if="mode === 'cbt'">
        <SvCard v-if="cbtPhase === 'form'">
          <h3 class="t">三栏书写</h3>
          <div class="field">
            <label>情境 · 只写事实，像监控摄像头拍到的</label>
            <textarea v-model="cbt.situation" rows="2" placeholder="昨晚开会时，我的发言被打断了…" />
          </div>
          <div class="field">
            <label>自动想法 · 当时脑子里冒出的那句话</label>
            <textarea v-model="cbt.autoThought" rows="2" placeholder="「我说的东西没人想听」" />
          </div>
          <div class="field">
            <label>替代想法 · 如果你的朋友这样讲，你会怎么为他辩护？</label>
            <textarea
              v-model="cbt.alternativeThought"
              rows="2"
              placeholder="「被打断是他的习惯，不代表我的话没价值」"
            />
          </div>
          <div class="field">
            <label>此刻情绪强度（0 最轻 - 10 最重）：{{ cbt.reappraisal }}</label>
            <input v-model.number="cbt.reappraisal" type="range" min="0" max="10" aria-label="情绪强度重评" />
          </div>
          <button class="sv-btn" @click="submitCbt">写给心屿看看</button>
          <SvDisclaimer
            class="disc"
            text="写下的内容会进入情绪梳理管道（信封加密）；追问是参考，不是评判。"
          />
        </SvCard>
        ...
        <SvCard v-else>
          ...
          <p v-else class="sv-muted">
            这次追问没生成，但你已经把想法从「自动化」写成了「可审视」——这本身就是收获。
          </p>
          <p class="sv-cap">
            今天情绪强度从写前的它，到现在的 {{ cbt.reappraisal }}/10。已自动计入今日跟练。
          </p>
        </SvCard>
      </template>
      ...
      <template v-else-if="mode === 'action'">
        <SvCard>
          ...
          <div class="tasks">
            <button
              v-for="t in ACTION_TASKS"
              :key="t"
              class="task"
              :class="{ on: picked.includes(t) }"
              :aria-pressed="picked.includes(t)"
              @click="
                picked.includes(t) ? picked.splice(picked.indexOf(t), 1) : picked.length < 2 && picked.push(t)
              "
            >
              {{ t }}
            </button>
      ...
      <template v-else>
        <SvCard>
          ...
          <div v-for="(s, i) in ex?.steps || []" :key="i" class="step" :class="{ on: stepDone[i] }">
            <label class="step-l">
              <input v-model="stepDone[i]" type="checkbox" :aria-label="s.step" />
              <span
                ><b>{{ i + 1 }}. {{ s.step }}</b>
                <p class="sv-muted">{{ s.desc }}</p></span
              >
            </label>
          </div>
          <div class="acts">
            <button class="sv-btn" :disabled="!allStepsDone || saving" @click="finishSteps(true)">
              全部完成，打卡
            </button>
...
.halo {
  ...
  background: radial-gradient(
    circle,
    color-mix(in srgb, var(--sv-mint) 38%, transparent),
    color-mix(in srgb, var(--sv-indigo) 22%, transparent) 70%
  );
...
.step.on {
  background: color-mix(in srgb, var(--sv-mint) 12%, var(--sv-card));
}
.step-l {
  display: flex;
  gap: var(--sv-s3);
