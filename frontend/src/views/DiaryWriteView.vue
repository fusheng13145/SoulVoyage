...
<template>
  <div class="write">
    ...
    <div class="body">
      <textarea v-model="text" rows="9" maxlength="5000" aria-label="今天的心情记录"
        placeholder="今天发生了什么？你感觉怎么样？写下来，让心屿陪你梳理一下……"></textarea>

      <div class="meta">
        <div class="moods" role="group" aria-label="今天心情 1 到 5 分">
          <button v-for="m in MOODS" :key="m.v" class="mood" :class="{ on: mood === m.v }"
            :aria-pressed="mood === m.v" :aria-label="m.t" @click="mood = mood === m.v ? 0 : m.v">{{ m.e }}</button>
        </div>
      ...
      <button class="sv-btn" :disabled="!canSubmit" @click="save">
        {{ submitting ? '正在交给心屿…' : (len < 10 ? '再多写一点吧（至少 10 字）' : '交给心屿梳理') }}</button>
...
textarea {
  width: 100%; border: 1px solid var(--sv-sep); background: var(--sv-card); border-radius: var(--sv-r-card);
  padding: 14px 16px; font-size: var(--sv-fs-body); resize: vertical; font-family: inherit;
  color: var(--sv-label); outline: none; line-height: 1.7;
}
textarea:focus { border-color: var(--sv-indigo); }
.meta { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin: var(--sv-s3) 0 var(--sv-s4); flex-wrap: wrap; }
.moods { display: flex; gap: 6px; }
.mood { border: 1.5px solid transparent; background: var(--sv-fill3); border-radius: var(--sv-r-ctl);
  font-size: 20px; min-width: 44px; min-height: 44px; cursor: pointer; opacity: .55; transition: .15s; }
.mood.on { opacity: 1; border-color: var(--sv-indigo); background: var(--sv-indigo-soft); }
.counter.short { color: var(--sv-amber); }
.err { color: var(--sv-red); font-size: var(--sv-fs-footnote); margin-bottom: var(--sv-s2); }
.hint { text-align: center; margin-top: var(--sv-s2); }
</style>
