...
async function cancelDeletion() {
  ...
  try {
    ...
    err.value = (e as Error).message || '撤回失败'
  }
}
...
async function changePassword() {
  ...
  try {
    ...
    err.value = (e as Error).message || '修改失败'
  }
}
...
async function exportData() {
  ...
  const ok = await confirmDialog({
    title: '导出全部个人数据？',
    message:
      '包含全部日记、报告、画像、练习与风险记录的 JSON 数据包。链接限时 5 分钟、仅能领取一次，请注意保管。',
    confirmText: '生成数据包',
  })
  if (!ok) return
  ...
  try {
    ...
    err.value = (e as Error).message || '导出失败'
  } finally {
...
async function requestDeletion() {
  ...
  try {
    ...
    err.value = (e as Error).message || '申请失败'
  }
}
</script>
    ...
    <div v-if="me" class="body">
      ...
      <SvCard v-if="me.status === 3">
        <div class="notice">
          ...
          <span
            >账号处于<b>注销冷静期</b>（申请于
            {{ me.deletionRequestedAt || '—' }}），到期后数据将被不可恢复地销毁。</span
          >
      ...
      <SvList title="我的数据">
        <SvCell :chevron="false" label="导出全部数据" icon="i-export" tone="var(--sv-indigo-soft)">
      ...
      <SvList title="修改密码">
        ...
        <div class="cell-pad">
          <button class="sv-btn" :disabled="!pw.old || !pw.next" @click="changePassword">
            修改并重新登录
          </button>
        </div>
      </SvList>
      ...
      <SvList>
        ...
        <SvCell
          :chevron="false"
          label="输入密码确认注销"
          hint="7 天冷静期内重新登录可撤回；到期后密钥销毁，所有加密内容永久无法解密"
          icon="i-trash"
