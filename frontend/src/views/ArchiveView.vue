...
async function load() {
  ...
  try {
    ...
    toast((e as Error).message || '档案加载失败')
  } finally {
    loading.value = false
  }
}
...
async function exportArchive() {
  ...
  try {
    ...
    toast((e as Error).message || '导出失败，链接限时 5 分钟且仅能领取一次')
  } finally {
    ...
    <div class="body">
      ...
      <template v-if="snapshot">
        <div class="print-only title-page">
          ...
          <p>
            生成时间 {{ snapshot.generatedAt.slice(0, 16).replace('T', ' ') }} UTC · 由心屿漫行自助平台整理
          </p>
        </div>
        <SvCard>
          ...
          <table class="tbl">
            ...
            <tbody>
              <tr v-for="w in snapshot.profiles" :key="w.statWeek">
                ...
                <td>{{ (w.stressorTop || []).map(s => `${s.name}×${s.count}`).join('、') || '—' }}</td>
              </tr>
            </tbody>
          </table>
        </SvCard>
        ...
        <button class="sv-btn no-print" @click="doPrint">
          <SvIcon name="i-download" :size="18" tone="inherit" /> 打印 / 存为 PDF
        </button>
      ...
      <template v-else-if="summary">
        <SvCard>
          <div class="week-head">
            ...
            <span v-if="summary.profile.avgValence" class="sv-cap"
              >平均效价 {{ summary.profile.avgValence }}</span
            >
          </div>
          <p v-if="summary.profile.stressorTop?.length" class="line sv-muted">
            压力源：<span v-for="s in summary.profile.stressorTop" :key="s.name" class="tag"
              >{{ s.name }} ×{{ s.count }}</span
            >
          </p>
          <p v-if="summary.profile.distortionTop?.length" class="line sv-muted">
            思维误区线索：<span v-for="d in summary.profile.distortionTop" :key="d.name" class="tag warm"
              >{{ d.name }} ×{{ d.count }}</span
            >
          </p>
        </SvCard>
        ...
        <SvList title="本周报告">
          ...
          <SvCell
            v-for="r in summary.reports"
            :key="r.id"
            :label="r.title"
            :hint="TYPE_LABELS[r.type] || r.type"
            icon="i-doc"
          ...
          <ul class="points">
            <li v-for="p in summary.emotionPoints" :key="p.date + p.emotion">
              ...
              <small>{{
                p.sourceType === 'DIARY'
                  ? '日记'
                  : p.sourceType === 'SIMULATION'
                    ? '模拟'
                    : p.sourceType === 'SELF_RATING'
                      ? '打卡'
                      : p.sourceType
              }}</small>
            </li>
        ...
        <SvCard v-if="summary.trainingScores?.length">
          ...
          <ul class="points">
            <li v-for="(t, i) in summary.trainingScores" :key="i">
              {{ t.date || '—' }} · 综合分 {{ t.avgScore }}
            </li>
          </ul>
        </SvCard>
        ...
        <button class="sv-btn ghost" :disabled="exporting" @click="exportArchive">
          <SvIcon name="i-export" :size="18" tone="inherit" />
          {{ exporting ? '生成快照中…' : '导出我的档案（限时一次性链接）' }}
        </button>

        <SvDisclaimer
          class="d"
          text="本档案为自助记录汇总，不构成任何医学诊断"
