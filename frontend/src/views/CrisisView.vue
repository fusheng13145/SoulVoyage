...
<template>
  <div class="crisis-page">
    <header>
      <router-link to="/today" class="back" aria-label="返回首页"
        ><SvIcon name="i-back" :size="18" tone="inherit" /> 返回</router-link
      >
    ...
    <main>
      <p class="intro">
        如果此刻很难熬，下面这些是真实、免费、可以随时连接的人和渠道。
        联系他们不代表「严重了」，只代表你愿意照顾自己。
      </p>
      ...
      <template v-else>
        ...
        <p class="sv-cap boundary">
          {{
            crisis.referral?.boundary ||
            '心屿漫行是自助工具，不做诊断，也不能替代专业帮助。转介即我们能陪你走的最远一步。'
          }}
        </p>
      </template>
...
main {
  max-width: 640px;
  margin: 0 auto;
  padding: var(--sv-s4) var(--sv-s4) var(--sv-s7);
}
.intro {
  color: var(--sv-label);
  font-size: var(--sv-fs-subhead);
...
.r-ico {
  ...
  flex: none;
  border-radius: 14px;
  background: color-mix(in srgb, var(--sv-red) 12%, transparent);
  color: var(--sv-red);
}
.r-mid {
  flex: 1 1 0;
...
.boundary {
  ...
  padding-top: var(--sv-s3);
  line-height: 1.7;
}
.warm {
  text-align: center;
  margin-top: var(--sv-s6);
  font-size: var(--sv-fs-footnote);
  color: var(--sv-label2);
}
</style>
