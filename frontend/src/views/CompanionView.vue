...
const moodHint = computed(() =>
  remaining.value !== null && remaining.value <= 20
    ? `今天还能聊 ${remaining.value} 句（明天我还在）` : '')

async function open() {
  loading.value = true
  try {
    const { data } = await http.post<ApiResp<any>>('/companion/sessions', {})
    ...
    for (const t of data.data.turnList || []) {
      ...
      msgs.value.push({ turnId: t.turnId, turnNo: t.turnNo, text: t.aiText, ai: true, crisis: t.crisis, noAnalyze: t.noAnalyze })
    }
    if (!msgs.value.length) {
      msgs.value.push({ text: GREET, ai: true })
    }
    if (ended.value) msgs.value.push({ text: '这一段已经收进行囊了。想继续聊，回到今日页点「来漫聊」开一段新的。', ai: true })
  } catch (e: any) {
    toast(e.message || '树洞暂时没打开')
  } finally {
    loading.value = false
    scroll()
  }
}
const GREET = '我在呢。这里没有打分也没有分析压力——想说什么就说，写完想撤也没关系（每条回复你都可以标「这句别分析」）。'
onMounted(open)
...
async function send() {
  ...
  try {
    await postSse(`/companion/sessions/${sessionId.value}/turns`, { userText: t }, (e) => {
      ...
      if (e.event === 'ai_delta') { ai.text += e.data.text; scroll() }
...
async function endChat() {
  ...
  sealed.value = true
  try {
    const { data } = await http.post<ApiResp<{ taskNo: string }>>(`/companion/sessions/${sessionId.value}/end`, {})
    msgs.value.push({ text: data.data.taskNo ? '收好啦。这段时间我会轻轻归档进你的情绪地图。' : '收好啦。想继续随时回来。', ai: true })
  } catch {
    sealed.value = false
    toast('刚才没收到，再点一次试试')
  }
...
<template>
  <div class="cpn">
    <SvNavBar title="漫聊" :subtitle="`树洞 · 第 ${segmentNo} 段`" back="今日" back-to="/today" :large="false">
      <template #actions>
        <button v-if="!ended && sessionId" class="end-btn" :disabled="sealed" aria-label="收起这段对话" @click="endChat">
    ...
    <div class="body">
      ...
      <SvCard v-if="crisis.crisisMode" :pad="false" class="care">
        <CrisisReferral level="MEDIUM"
          headline="这段时间漫聊会一直陪着你——不用组织语言，说碎片也可以" />
      </SvCard>
      ...
      <div ref="chatBox" class="chat sv-scroll" aria-live="polite">
        <template v-for="(m, i) in msgs" :key="i">
          ...
          <CrisisReferral v-if="m.crisis" level="HIGH"
      ...
      <SvDisclaimer text="漫聊是低压力陪伴空间：回复由 AI 生成，不替代专业咨询；危机信号会当轮触发求助资源。"
        reason="你可以随时点「这句别分析」让某句话留在当下——收段后的情绪消化，永远听你的。" />

      <div v-if="!ended" class="input-row">
        <textarea v-model="draft" rows="2" :disabled="streaming || sealed" aria-label="想说什么"
          placeholder="说点什么…不想写全也没关系" @keydown.enter.exact.prevent="send"></textarea>
...
<style scoped>
.body { padding: 0 var(--sv-s4); display: flex; flex-direction: column; }
.wait { text-align: center; padding: var(--sv-s5) 0; }
.care { margin-bottom: var(--sv-s3); }
.chat { display: flex; flex-direction: column; gap: 10px; background: var(--sv-bg);
  border-radius: var(--sv-r-card); padding: var(--sv-s3); box-shadow: inset 0 0 0 1px var(--sv-sep);
  min-height: 40vh; max-height: 52vh; overflow-y: auto; margin-bottom: var(--sv-s3); }
.hide { margin-left: 10px; border: none; background: transparent; color: var(--sv-label3);
  font-size: var(--sv-fs-caption2); cursor: pointer; text-decoration: underline; padding: 0; font-family: inherit; }
.hide.done { cursor: default; text-decoration: none; color: var(--sv-mint); }
.hint { text-align: center; margin-bottom: var(--sv-s2); }
.end-btn { display: grid; place-items: center; width: 44px; height: 44px; border: none;
  background: transparent; color: var(--sv-indigo); cursor: pointer; }
...
.send { width: 44px; height: 44px; flex: none; border-radius: 50%; border: none; cursor: pointer;
  display: grid; place-items: center; background: var(--sv-indigo); color: #fff; transition: transform .12s var(--sv-ease); }
