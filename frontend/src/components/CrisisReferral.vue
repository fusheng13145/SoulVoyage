...
<template>
  <div
    ...
    role="note"
    :aria-label="'支持资源（' + (level === 'HIGH' ? '紧急' : '一般') + '）'"
  >
    <b>{{ title }}</b>
    ...
    <p class="boundary">
      {{ crisis.referral?.boundary || '平台为自助工具，不做诊断。紧急情况请拨打 110/120。' }}
    </p>
...
[data-theme='dark'] .referral.high {
  background: color-mix(in srgb, var(--sv-red) 16%, var(--sv-card));
  color: var(--sv-label);
}
[data-theme='dark'] .referral.medium {
  background: color-mix(in srgb, var(--sv-amber) 14%, var(--sv-card));
  color: var(--sv-label);
}
.referral b {
  display: block;
