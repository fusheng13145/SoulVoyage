      ...
      <div v-if="dialogState" class="wrap" @keydown="onKey" role="presentation">
        ...
        <div class="box" role="alertdialog" aria-modal="true" :aria-label="dialogState.title">
          ...
          <div class="btns">
            ...
            <button class="ok" :class="{ danger: dialogState.danger }" data-autofocus
...
<style scoped>
.wrap { position: absolute; inset: 0; z-index: 130; display: grid; place-items: center; }
.veil { position: absolute; inset: 0; background: var(--sv-dim); }
.box {
  position: relative; width: min(280px, 84vw);
  background: color-mix(in srgb, var(--sv-card) 92%, transparent);
  backdrop-filter: var(--sv-blur); -webkit-backdrop-filter: var(--sv-blur);
  border-radius: var(--sv-r-ctl); text-align: center; overflow: hidden;
  box-shadow: var(--sv-sh-2);
}
[data-theme="dark"] .box { border: 1px solid var(--sv-sep); }
.box b { display: block; padding: 19px 16px 5px; font-size: var(--sv-fs-headline); }
.box p { padding: 0 16px 19px; font-size: var(--sv-fs-callout); color: var(--sv-label2); line-height: 1.4; }
.btns { display: grid; grid-template-columns: 1fr 1fr; border-top: 1px solid var(--sv-sep); }
.btns button {
  min-height: 44px; border: none; background: transparent; cursor: pointer;
  font-size: var(--sv-fs-headline); color: var(--sv-blue); font-weight: 600;
  display: inline-flex; align-items: center; justify-content: center; gap: 4px;
}
.btns button + button { border-left: 1px solid var(--sv-sep); }
.btns .ok.danger { color: var(--sv-red); }
.dlg-enter-active, .dlg-leave-active { transition: opacity 0.25s; }
