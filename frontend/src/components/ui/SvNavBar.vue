...
<template>
  <div class="navw">
    <div class="glass" :class="{ show: large && collapsed, flat: !large }">
      <div class="bar">
        <button v-if="showBack" class="back" :aria-label="typeof back === 'string' ? back : '返回'"
          @click="backTo ? $router.push(backTo) : $router.back()">
          <SvIcon name="i-back" :size="20" tone="inherit" /><span v-if="typeof back === 'string'">{{ back }}</span>
        </button>
...
.glass.show, .glass.flat {
  background: color-mix(in srgb, var(--sv-bg) 72%, transparent);
  backdrop-filter: var(--sv-blur); -webkit-backdrop-filter: var(--sv-blur);
}
.glass.show { border-bottom-color: var(--sv-sep); }
.bar {
  position: relative;
  min-height: 44px; padding: var(--sv-safe-t) 6px 0;
  display: flex; align-items: center; gap: 8px;
}
.large { padding: calc(var(--sv-safe-t) + 8px) 6px 6px; }
.large h1 {
  font-size: var(--sv-fs-largename); font-weight: 700; letter-spacing: 0.4px;
  transition: opacity 0.3s; line-height: 1.25;
}
.large h1 small { display: block; font-size: var(--sv-fs-footnote); font-weight: 400; color: var(--sv-label2); letter-spacing: 0; margin-top: 2px; }
.back {
  border: none; background: transparent; cursor: pointer; color: var(--sv-indigo);
  font-size: var(--sv-fs-subhead); display: inline-flex; align-items: center; gap: 2px;
  padding: 8px 6px; min-height: 44px; font-family: inherit;
}
.ctitle {
  flex: 1; text-align: center; padding: 0 8px;
  font-weight: 600; font-size: var(--sv-fs-headline);
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  opacity: 0; pointer-events: none; transition: opacity 0.25s;
}
.ctitle.show { opacity: 1; }
.acts { margin-left: auto; display: inline-flex; align-items: center; gap: 10px; }
</style>
